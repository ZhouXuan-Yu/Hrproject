    ...
    <div class="card" style="margin-bottom:12px">
      ...
      <div class="candidate-filter" style="margin-bottom:12px">
        ...
        <select v-model="matchTopN" style="padding:3px 6px;border:1px solid var(--c-border);border-radius:4px;font-size:12px;width:60px">
          <option :value="MATCH_TOP_N_ALL">全部</option>
...
const loadError = ref('');
const MATCH_TOP_N_ALL = 9999; // 「推荐人数」=「全部」时的哨兵值
const matchTopN = ref(Number(localStorage.getItem('demand_match_topn')) || MATCH_TOP_N_ALL);
// 记住上次选择的「推荐人数」，刷新不重置
watch(matchTopN, (v) => { try { localStorage.setItem('demand_match_topn', String(v)); } catch (e) {} });
...
const visibleCandidates = computed(() => {
  const limit = matchTopN.value || MATCH_TOP_N_ALL;
